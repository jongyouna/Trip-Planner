# Firebase 설정 (Google 로그인 + 최저가 탐색)

대시보드는 GitHub Pages 정적 사이트다. 로그인은 Firebase Auth(클라이언트), 권한 강제는 Firestore 보안 규칙,
수집은 자택 PC의 워커(`npm run worker`)가 맡는다. Firebase 프로젝트는 `buja-map-vercel`과 같은 `buja-map-b52eb`를 쓴다.

```
브라우저 --Google 로그인--> Firebase Auth
...
1. **승인 도메인**: Firebase 콘솔 > Authentication > Settings > Authorized domains에 `jongyouna.github.io` 추가.
   (**완료** — 2026-09-22 확인. `localhost`는 기본 포함. Google 로그인 제공자는 buja-map에서 이미 켜져 있다.)
2. **Firestore 규칙**: `buja-map-vercel/firestore.rules`의 `match /config/{docId}` 블록 아래에 다음을 **추가**하고
...
3. **서비스 계정 키**: 콘솔 > 프로젝트 설정 > 서비스 계정 > "새 비공개 키 생성". JSON을 **리포 밖**(예: `C:\Users\jongy\secrets\`)에
